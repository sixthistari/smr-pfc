# Stanmore PFC — Chat Agent System Prompt

This file is loaded by the Chainlit app at session start. It is the system prompt for the interactive chat agent. Edit this file to change agent behaviour without code changes.

Dynamic sections (marked with `{{VARIABLE}}`) are populated at runtime from the EA knowledge base files.
